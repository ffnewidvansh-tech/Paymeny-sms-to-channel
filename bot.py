...
def send_auto_sms():
    """Bot khud apne aap SMS bhejega (simulate)"""
    while True:
        try:
            # Wait 10 second
            time.sleep(10)
            ...
            # Bot khud ko message bhejega (simulate)
            bot.send_message(
                chat_id=os.environ.get('BOT_OWNER_ID', '123456789'),  # Owner ID
                text=sms_text
            )
...
def send_test(message):
    ...
    bot.reply_to(message, f"📤 Sending test SMS:\n`{test_sms}`", parse_mode="Markdown")
    # Bot khud ko message bhejega (simulate)
    bot.send_message(
        chat_id=message.chat.id,
        text=test_sms
...
# ==================== FORWARD SMS (Sirf Bot Ke Khud Ke Messages) ====================
...
def forward_to_group(message):
    ...
    # SIRF BOT KE KHUD KE MESSAGES FORWARD HOGE
    # Check: Agar message bot ne khud bheja hai toh forward karo
    if message.from_user.id != bot.get_me().id:
        # Agar kisi aur ne bheja hai toh ignore karo
        bot.reply_to(message, "❌ Sirf bot ke khud ke messages forward honge! (Auto SMS)")
        return
    ...
    try:
        logger.info(f"📩 Bot's own message: {message.text[:50]}...")
